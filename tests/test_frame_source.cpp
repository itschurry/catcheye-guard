...
{
    return fs::temp_directory_path() / "catcheye_frame_source_test.png";
}
...
TEST_CASE(image_file_source_reads_once_then_reaches_end_of_stream)
{
    ...
    catcheye::InputSourceConfig config;
    config.type = catcheye::InputSourceType::ImageFile;
    config.uri = image_path.string();
    catcheye::ImageFileSource source(config);

    test_support::assert_true(source.open(), "image source open should succeed");

    catcheye::Frame frame;
    test_support::assert_true(source.read(frame) == catcheye::FrameReadStatus::Ok, "first read should succeed");
    ...
    test_support::assert_true(source.read(frame) == catcheye::FrameReadStatus::EndOfStream, "second read should end");
...
{
    catcheye::InputSourceConfig config;
    config.type = catcheye::InputSourceType::VideoFile;
    config.uri = "/tmp/catcheye_missing_video.mp4";
    catcheye::OpenCvCaptureSource source(config);
...
{
    catcheye::InputSourceConfig config;
    config.type = catcheye::InputSourceType::Camera;

...
bool HttpRoiServer::start() {
    ...
    running_ = true;
    accept_thread_ = std::thread(&HttpRoiServer::accept_loop, this);
...
void HttpRoiServer::accept_loop() {
    pollfd pfd{};
    pfd.fd = server_fd_;
    pfd.events = POLLIN;
...
void HttpRoiServer::handle_client(int client_fd) {
    std::string request;
    std::string body;
...
bool HttpRoiServer::send_response(int client_fd, int status_code, const std::string& status_text, const std::string& body) const {
    std::ostringstream oss;
...
bool HttpRoiServer::handle_get_roi(int client_fd) {
    ...
    if (!parse_result.success) {
        return send_response(
            ...
            json_error_body("failed to load ROI config file", parse_result.errors));
    }
    ...
    if (!validation.valid) {
        return send_response(
            ...
            500,
            "Internal Server Error",
            json_error_body("ROI config file is invalid", validation_issue_messages(validation)));
    }
...
bool HttpRoiServer::handle_put_roi(int client_fd, const std::string& body) {
    ...
    if (!parse_result.success) {
        return send_response(
            ...
            400,
            "Bad Request",
            json_error_body("failed to parse ROI JSON", parse_result.errors));
    }

...
class HttpRoiServer {
  public:
    HttpRoiServer(HttpRoiServerConfig config, std::string roi_config_path, GuardProcessor* processor);
    ~HttpRoiServer();
  ...
  private:
    ...
    bool handle_get_roi(int client_fd);
    bool handle_put_roi(int client_fd, const std::string& body);
